...
def deepFaceInterVideo():
    ...
    for model in model_names:
        for dirpath1, dirnames1, filenames1 in os.walk(params.path):
            filenames1 = [f for f in filenames1 if not f[0] == '.' and f[-18:] == 'deepFaceVector.txt' and f.split("_")[2] == model]
            for file1 in sorted(filenames1):
                ...
                fileOutput = open(dirpath1 + "/" + timeFile1 + "_" + id1 + "_" + model + "_deepFaceInterResult.txt", "w")
                for dirpath2, dirnames2, filenames2 in os.walk(params.path):
                    filenames2 = [f for f in filenames2 if not f[0] == '.' and f[-18:] == 'deepFaceVector.txt' and f.split("_")[2] == model]
                    for file2 in sorted(filenames2):
                        ...
                        if place1 != place2:
                            vectorFeature2 = []
                            fileOutput2 = open(dirpath2 + "/" + timeFile2 + "_" + id2 + "_" + model + "_deepFaceInterResult.txt")
