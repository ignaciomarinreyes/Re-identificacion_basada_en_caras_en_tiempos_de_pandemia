...
def rank1AlignedReId():
    ...
    for modelName in model_names:
        ...
        denominadorRank1 = 0
        for dirpath1, dirnames1, filenames1 in os.walk(params.path):
            filenames1 = [f for f in filenames1 if not f[0] == '.' and f[-23:] == 'AlignedReIdDistance.txt' and f.split("_")[5] == modelName.split("_")[0]]
            for file1 in sorted(filenames1):
                distAlignList = []
                distOriginList = []
                id2List = []
...
def alignedReIdVector():
    ...
    model_names = ["Cuhk03_Resnet50", "DukeMTMCReID_Resnet50", "Market1501_Resnet50", "MSMT17_Resnet50"]
    num_classes_models = [767, 702, 751, 1041]
    for modelName, num_classes in zip(model_names, num_classes_models):
        print("======= " + modelName + " ==========")
        os.environ['CUDA_VISIBLE_DEVICES'] = "0"
        use_gpu = torch.cuda.is_available()
        model = models.init_model(name='resnet50', num_classes=num_classes, loss={'softmax', 'metric'}, use_gpu=use_gpu,aligned=True)
        ...
        for dirpath, dirnames, filenames in os.walk(params.path):
            ...
            for file in sorted(filenames):
                ...
                np.savez(dirpath + "/" + timeFile + "_" + id + "_" + modelName + "_AlignedReId",a1[0],a1[1],a1[2],a1[3],a1[4],a1[5],a1[6],a1[7])
...
def distanceAlignedReIdVector():
    ...
    for modelName in model_names:
        print("======= " + modelName + " ==========")
        for dirpath1, dirnames1, filenames1 in os.walk(params.path):
            filenames1 = [f for f in filenames1 if not f[0] == '.' and f[-15:] == 'AlignedReId.npz' and f.split("_")[5] == modelName.split("_")[0]]
            for file1 in sorted(filenames1):
                ...
                fileOutput = open(dirpath1 + "/" + timeFile1 + "_" + id1 + "_" + modelName + "_AlignedReIdDistance.txt", "w")
                for dirpath2, dirnames2, filenames2 in os.walk(params.path):
                    filenames2 = [f for f in filenames2 if not f[0] == '.' and f[-15:] == 'AlignedReId.npz' and f.split("_")[5] == modelName.split("_")[0]]
                    for file2 in sorted(filenames2):
                        ...
                        if place1 != place2:
                            arrays2 = np.load(dirpath2 + "/" + timeFile2 + "_" + id2 + "_" + modelName + "_AlignedReId.npz")
