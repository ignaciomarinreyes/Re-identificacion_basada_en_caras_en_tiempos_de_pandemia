...
for pathTxtBody, pathJpg in zip(sorted(glob.glob(path + "*.txt")), sorted(glob.glob(path + "*.jpg"))):
    print("###################################")
    ...
    for line in fileBody:
        ...
        if (len(values) == 6 and values[4] == '1'):
            ...
            crop_img = img[beginY:endY, beginX:endX]
            ...
            if faces is not None:
                ...
                for i in range(faces.shape[0]):
                    ...
                    if mask >= mask_thresh:
                        ...
                        color = (0, 255, 0)
                    cv2.rectangle(crop_img, (box[0], box[1]), (box[2], box[3]), color, 2)
                    landmark5 = landmarks[i].astype(np.int)
                    for l in range(landmark5.shape[0]):
                        color = (255, 0, 0)
                        cv2.circle(crop_img, (landmark5[l][0], landmark5[l][1]), 1, color, 2)
                cv2.imwrite(path + "Salida_frame_" + timeFile + "_result_" + str(j) + ".jpg", crop_img)
